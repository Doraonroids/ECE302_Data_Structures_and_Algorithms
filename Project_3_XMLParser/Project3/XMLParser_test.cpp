...
TEST_CASE( "Test constuctor", "[XMLParser]"){
	XMLParser myXMLParser;
	bool success;
	success = myXMLParser.containsElementName("");
	REQUIRE(!success);
}
...
TEST_CASE( "Test improper <>", "[XMLParser]"){
	 INFO("Hint: tokenize nested elements test of XMLParse");
		// Create an instance of XMLParse
		XMLParser myXMLParser;
		string testString = "<?xml version=\"1.0\" encoding=\"UTF-8\"?><Note src='gmail'>  <From>Tom</From> <To>Alice</To> </Note>";
		int ct = 0;
		...
		for(unsigned int i = 0; i < testString.length(); i ++){
			...
			if(ct == 1 ){
				switch(i){
					case 37:
					case 38: REQUIRE(success); break;
					case 55:
					case 56: REQUIRE(success); break;
					case 63:
					case 64: REQUIRE(success); break;
					case 73:
					case 74: REQUIRE(success); break;
					case 88:
					case 89: REQUIRE(success); break;
					case 94:
					case 96: REQUIRE(success); break;
				}
			}else{
				REQUIRE(!success);
			}
...
TEST_CASE( "Test XMLParser parseTokenizedInput fail for tag name", "[XMLParser]" )
{
		// Create an instance of XMLParse
		XMLParser myXMLParser;
		char test = 0;
		for(unsigned int i = 0; i < 42; i++){
			test = i;
			string testString = "<test myattr='abcdef'>stuff<this_is";
			testString += test;
			testString += "_empty_tag/></test>";
...
TEST_CASE( "Test XMLParser parseTokenizedInput fail for first character tag name", "[XMLParser]" )
{
		// Create an instance of XMLParse
		XMLParser myXMLParser;
		char test = 0;
		for(unsigned int i = 48; i < 57; i++){
			string testString =  "<test myattr='abcdef'>stuff<";
			test = i;
			testString += test;
		
			testString += "this_is_empty_tag/></test>";
...
TEST_CASE( "Test XMLParser parseTokenizedInput Handout-0", "[XMLParser]" )
{
	   INFO("Hint: tokenize single element test of XMLParse");
		...
		for (int i = 0; i < result.size(); i++) {
			REQUIRE(result[i].tokenType == output[i].tokenType);
			REQUIRE(result[2].tokenString.compare(output[2].tokenString) == 0);
		}
}
